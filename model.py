...
class ft_net_VGG16(nn.Module):
    ...
    def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='gem', feature_dim=2048):
    # def __init__(self, class_num, droprate=0.5, stride=2, init_model=None, pool='gem'):
        ...
        # # Feature extractor
        self.feature_extractor = nn.Sequential(*list(model_ft.features.children()))

        # # Optional bottleneck layer to project features to the desired size
        # self.bottleneck = nn.Sequential(
        #     nn.Linear(512 if pool != 'avg+max' else 1024, feature_dim),
        ...
        # # Initialize from another model if provided
        # if init_model is not None:
        #     self.feature_extractor = init_model.feature_extractor
        #     self.pool = init_model.pool
        #     self.bottleneck = init_model.bottleneck
...
class three_view_net(nn.Module):
    def __init__(self, class_num, droprate, stride = 2, pool = 'avg', share_weight = False, VGG16=False, circle=False):
        ...
        # Dynamically determine the feature size
        sample_input = torch.randn(1, 3, 256, 256)  # Example input size
        feature_size = self._get_feature_size(self.model_1, sample_input)
        
        # Initialize the classifier with the correct input size
        self.classifier = ClassBlock(feature_size, class_num, droprate, return_f=circle)
        if pool == 'avg+max':
            self.classifier = ClassBlock(feature_size * 2, class_num, droprate, return_f=circle)

    def _get_feature_size(self, model, sample_input):
        """Helper function to determine the feature size dynamically."""
        with torch.no_grad():

...
parser.add_argument('--pool', default='gem', type=str, help='pool gem')
        ...
        for phase in ['train']:
            ...
            for data, data2, data3, data4 in zip(dataloaders['satellite'], dataloaders['street'], dataloaders['drone'],
                ...
                if opt.views == 2:
                    ...
                    _, preds2 = torch.max(outputs2[0].data, 1)
                    loss = criterion(outputs[0], labels) + criterion(outputs2[0], labels2)
                elif opt.views == 3:
                    if return_feature:
                        logits, ff = outputs
